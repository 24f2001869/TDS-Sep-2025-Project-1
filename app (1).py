    ...
    for i, delay in enumerate(delays):
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"⚠️ Notification failed with status {response.status_code}. Retrying...")
        except requests.RequestException as e:
            print(f"🚨 Notification request failed: {e}. Retrying...")
...
def background_task(task_data):
    ...
    print(f"🤖 Starting background job for task: {task_data.get('task')}, Round: {task_data.get('round')}")
    ...
    try:
        ...
        if task_data.get("secret") != MY_SECRET:
            print("❌ Secret mismatch. Aborting task.")
            return
